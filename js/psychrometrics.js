...
// tdb = $('#ta').val()
// hum = $('#rh').val()
// psy.convert(hum, tdb, window.humUnit, 'rh')
...
psy.tdb_twb = function(tdb, twb){
  ...
  a.wetbulb = twb;
  a.w = ((PROP.Hfg + (PROP.CpVap - PROP.CpWat) * twb) * wstar - 
          PROP.CpAir * (tdb - twb)) /
...
psy.tdb_dewpoint = function(tdb, dewpoint){
  var w_l=0.00001, w_r=0.2, eps=0.0001;
  var fn = function(w){
    var dpstar = psy.dewpoint(w);
    return dewpoint - dpstar;
  }
  var w = this.bisect(w_l, w_r, fn, eps, 0);
...
psy.wetbulb = function(tdb, w){
  var psatStar, wStar, fn, eps=0.01, wetbulb_l=-100, wetbulb_r=150;
  var PROP = this.PROP;
  
  fn = function(t){
    psatStar = psy.satpress(t);
    wStar = psy.humratio(PROP.Patm ,psatStar);
    newW = ((PROP.Hfg - PROP.CpWat - PROP.CpVap * t) * wStar - 
           PROP.CpAir * (tdb - t)) / (PROP.Hfg + PROP.CpVap * tdb - 
           PROP.CpWat * t);
    return (w - newW);   
  }
  return this.bisect(wetbulb_l, wetbulb_r, fn, eps, 0);
}
...
psy.sattemp = function(p){
  var tsat_l = 0, tsat_r = 500, eps = 0.0001, psat;
  var fn = function(t){
    return (p - psy.satpress(t));
  }

  return this.bisect(tsat_l, tsat_r, fn, eps, 0);
}

psy.tairsat = function(hsat){
  var tsat_l = 0, tsat_r = 1000, eps = 0.01;
  var fn = function(t){
...
psy.bisect = function(a, b, fn, epsilon, target){
