...
function between(x,l,r){
  return (x > l && x < r);
}

function globeTemperature(tw, tr, ta){
  // calculate composite globe temperature
  return 0.7 * tw + 0.2 * tr + 0.1 * ta;
}

function adaptiveComfortASH55(ta, tr, runningMean, vel){
  var coolingEffect = 0;
  if (vel > 0.15 & runningMean >= 25){
    // calculate cooling effect of elevated air speed
    // when the outdoor running mean temp > 25 degC.
    var _rh = 50, _met = 1, _clo = 0.5, _wme = 0;
    var SETElevated = pierceSET(ta, tr, vel, _rh, _met, _clo, _wme);
    var SETStillAir = pierceSET(ta, tr, 0.15, _rh, _met, _clo, _wme);
    var coolingEffect = SETStillAir - SETElevated;
  }
  ...
  if (between(to,tComf90Lower,tComf90Upper)){
    // compliance at 80% and 90% levels
    acceptability80 = acceptability90 = true;
  }else if (between(to,tComf80Lower,tComf80Upper)){
  ...
  return [[acceptability80,tComf80Lower,tComf80Upper],
          [acceptability90,tComf90Lower,tComf90Upper]];
}

function pmvElevatedAirspeed(ta, tr, vel, rh, met, clo, wme){
  // returns pmv at elevated airspeed (>0.15m/s)
  if (vel <= 0.15){
	var SET = pierceSET(ta, tr, vel, rh, met, clo, wme);
	return [pmv(ta, tr, vel, rh, met, clo, wme), SET, SET, 0.0]
  }else{
	var SETElevated = pierceSET(ta, tr, vel, rh, met, clo, wme);
	var SETStillAir = pierceSET(ta, tr, 0.15, rh, met, clo, wme);
	var coolingEffect = SETStillAir - SETElevated;
    return [pmv(ta-coolingEffect, tr-coolingEffect, 0.15, rh, met, clo, wme), SETElevated, SETStillAir, coolingEffect]
  }
}
...
function pierceSET(ta, tr, vel, rh, met, clo, wme){
  ...
  var VaporPressure = rh * FindSaturatedVaporPressureTorr(ta) / 100;
  ...
  if (met < 0.85)
    ...
    CHCA = 5.66 * pow(((met - 0.85) * PressureInAtmospheres),0.39);
  ...
  for (TIM=1; TIM<=LTIME; TIM++){        
    ...
    WARMS = POSVALUE(SKSIG);
    COLDS = POSVALUE(-1.0 * SKSIG);
    CRSIG = (TempCore - TempCoreNeutral);
    WARMC=POSVALUE(CRSIG);
    COLDC=POSVALUE(-1.0 * CRSIG);
    BDSIG= TB - TempBodyNeutral;
    WARMB=POSVALUE(BDSIG);
    COLDB=POSVALUE(-1.0 * BDSIG);
  ...
  var dx = 100.0;
  while (abs(dx)>.01){
    ERR1 = (HSK - HD * (TempSkin - X_OLD) - W * HE * (PSSK - 0.5 * FindSaturatedVaporPressureTorr(X_OLD))); 
    ERR2 = (HSK - HD * (TempSkin - (X_OLD + DELTA)) - W * HE * (PSSK - 0.5 * FindSaturatedVaporPressureTorr((X_OLD + DELTA)))); 
